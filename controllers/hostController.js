...
// Used by frontend to get host's location for "Artists Nearby"
exports.getHostProfile = async (req, res) => {
    ...
    try {
        ...
        if (hostRows.length === 0) {
            ...
            return res.status(200).json({
                ...
                profile: {
                    ...
                    location: 'Not Set',
                    // New split fields for frontend convenience
                    district: 'Not Set',
        ...
        // Derive district/city from legacy single location string if present
        let derivedDistrict = 'Not Set';
        let derivedCity = 'Other / Not listed';
        if (formattedProfile.location && typeof formattedProfile.location === 'string') {
            const split = String(formattedProfile.location).split(/[\-,]/).map(s => s.trim()).filter(Boolean);
            if (split.length >= 2) {
                derivedDistrict = split[0] || 'Not Set';
                derivedCity = split[1] || 'Other / Not listed';
            } else if (split.length === 1) {
                // If only one value present, assume it's the district
                derivedDistrict = split[0] || 'Not Set';
            }
        }

        // Attach split fields (may be used directly by frontend)
        formattedProfile.district = derivedDistrict;
        formattedProfile.city = derivedCity;
...
exports.updateHostProfile = async (req, res) => {
    upload(req, res, async (err) => {
        ...
        const {
            ...
            contact_number,
            location,
            // New split fields from frontend; we'll compute a combined location
            district,
            city,
        ...
        try {
            ...
            // Build final location value using priority: provided location, or `${district} - ${city}`, else 'Not Set'
            let finalLocationValue = (location && String(location).trim() !== '') ? String(location).trim() : null;
            if (!finalLocationValue) {
                const d = (district && String(district).trim() !== '') ? String(district).trim() : null;
                const c = (city && String(city).trim() !== '') ? String(city).trim() : null;
                if (d && c) {
                    finalLocationValue = `${d} - ${c}`;
                } else if (d) {
                    finalLocationValue = d;
                } else if (c) {
                    finalLocationValue = c;
                } else {
                    finalLocationValue = 'Not Set';
                }
            }
